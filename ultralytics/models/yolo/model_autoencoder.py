...
from pathlib import Path
import torch
from ultralytics.nn.modules.losses_ae import structural_loss, ssim_loss, perceptual_loss, hybrid_loss
from ultralytics.nn.modules.ae import AutoEncoder  # AE 정의된 위치에 따라 수정 필요
...
class YOLO(Model):
    ...
    def __init__(self, model: Union[str, Path], task=None, verbose=False, fold=None):
        ...
        if "-world" in path.stem and path.suffix in {".pt", ".yaml", ".yml"}:  # if YOLOWorld PyTorch model
        ...
        elif "_ae" in path.stem and path.suffix in {".pt", ".yaml", ".yml"}:
            ...
            # 🔽 여기서 디버깅 코드를 넣어
            from ultralytics.models.yolo.yoloe.head import YOLOEDetect  # 정확한 위치로 임포트
...
# model_autoencoder.py (또는 적절한 위치)
import torch.nn.functional as F
from ultralytics.nn.modules.ae import AutoEncoder  # AE 구조 정의한 파일
...
class YOLOEAutoEncoder(Model):
    """
    YOLOE + AutoEncoder 결합 모델.
    입력 1채널 이미지 → AE → 3채널 변환 후 YOLOE 검출기로 전달.
    """
    def __init__(
        self,
        model: str      = "yoloe.yaml",
        task:  str      = None,
        verbose: bool   = False,
        fold:  int      = None,
        ae_loss_fn      = None,
        ...
        # 1) AE 모델 생성
        self.autoencoder = AutoEncoder(loss_type="hybrid")

        # 2) 1-based fold 인자로 weight 로드
        if fold is not None:
            ae_path = Path(f"ae/pretrained_folds/pretrained_ae_fold{fold:02d}.pth")
            if ae_path.exists():
        ...
        # 3) AE 모듈을 detection backbone device 에 맞추고 train 모드로
        device = self.model.model[0].conv.weight.device
        self.autoencoder.to(device)
        self.autoencoder.train()

        # 4) AE loss 함수 설정
        self.ae_loss_fn = ae_loss_fn or F.l1_loss

        # 5) 클래스명(names) 초기화 (YOLODetect 에서 사용)
        if not hasattr(self.model, "names"):
            self.model.names = YAML.load((ROOT / "cfg/datasets/coco8.yaml"))["names"]

    @property
    def task_map(self):
        """YOLOE 구조 동일하게 detect/segment 작업 매핑"""
        return {
            "detect": {
                "model": YOLOEModel,
    ...
    def forward(self, x, *args, **kwargs):
        """AE 통과 후 YOLOE forward 호출"""
        x_ae = self.autoencoder(x)  # [1ch → AE → 3ch]
        return self.model(x_ae, *args, **kwargs)

    def compute_ae_loss(self, x):
        """AE reconstruction loss 계산 (train용)"""
        _, loss = self.autoencoder(x, return_loss=True)
        return loss

    def val(self, validator=None, **kwargs):
        """YOLOE 스타일 검증 함수 (text prompt 기반 val 대응)"""
    ...
    def predict(self, source=None, stream=False, predictor=None, **kwargs):
        ...
        self.predictor = (predictor or self._smart_load("predictor"))(
            overrides={**self.overrides, **kwargs, "mode": "predict"},
            _callbacks=self.callbacks
        )
